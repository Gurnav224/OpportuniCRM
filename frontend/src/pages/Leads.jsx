...
const Leads = () => {
  ...
  const filterByStatus = (e) => {
    ...
    setFilterLeads(leads.filter((lead) => lead.status === value));
  };


const filterBySalesAgent = (e) => {
  const {value } = e.target;
  setFilterLeads(leads.filter((lead) => lead.salesAgent.name === value))
}

const sortByPriority = () => {
  let priorityOrder = {High:0, Medium:1,Low:2};
  setFilterLeads((prev) => {
    const sorted = [...prev].sort((a,b) => priorityOrder[a.priority] - priorityOrder[b.priority]);
    return sorted
  })
}
...
const sortByTimeToClose = () => {
  ...
  return (
    ...
    {/* Sidebar */}
      <div className="w-40">
        <Link  className="py-4  text-2xl font-medium text-blue-600 dark:text-blue-500 hover:underline" to={"/"}>
          Back To Dashboard
        </Link>
      </div>
      {/* Lead List */}
      <div className="border w-full">
        ...
        <ul className="w-full bg-white shadow-md rounded-lg overflow-hidden">
          ...
          {filterLeads.map((lead) => (
            <li key={lead._id} className="grid grid-cols-5 py-3 px-4 border-b hover:bg-gray-50 text-gray-800 text-left">
        ...
        <h4 className="font-bold">Filters: </h4>
          ...
          <div className="py-1 px-3 flex gap-3">
            <div className="flex gap-3 items-center">
              <label className="block" htmlFor="salesAgent">Filter By Sales Agent</label>
              <select onChange={(e) => filterBySalesAgent(e)} className="border py-2 px-2" name="salesAgent" id="salesAgent">
                <option value="">select</option>
                {
                  agents.map((agent) => (
        ...
        <Link  to={"/newlead"} className="px-3 py-3 block font-medium text-blue-600 dark:text-blue-500 hover:underline">
              Add New Lead
            </Link>  
      </div>
    </div>
